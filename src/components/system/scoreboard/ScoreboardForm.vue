...
import { insertScoreboardData, fetchScoreboardOptions } from '@/api/scoreboard';
...
const prescribedPeriodValues = computed(() => {
    ...
    if (Array.isArray(typesOfTransaction.value) && formData.value.typeOfTransaction?.length !== 0 && formData.value.typeOfTransaction !== null) {
...
const handleDialogFormSubmit = (formDialogData) => {
    ...
    const reportsData = formData.value.reportsData.filter((reportData) => (
        reportData.report.report_id !== formDialogData.report.report_id
    ))
        ...
        <v-card-text>
            <v-form ref="refVForm" @submit.prevent="handleFormSubmit">
                <v-row>
                    ...
                    <v-col>
                        <v-select label="Choose TS in Charge" :items="options.tsInCharge" item-title="name"
                            item-value="tic_id" :rules="[requiredValidator]" outlined v-model="formData.particulars.ts">
                        </v-select>
                    </v-col>
                </v-row>
                <v-row>
                    <v-col> <v-text-field :rules="[requiredValidator]" label="Agency Name"
                            v-model="formData.particulars.agency" outlined clearable>
                        </v-text-field>
                    </v-col>
                    <v-col>
                ...
                <v-row>
                    <v-col>
                        <v-text-field :rule="requiredValidator" label="DMS Reference Number"
                            v-model="formData.dmsReferenceNumber" outlined clearable>
                        </v-text-field>
                    </v-col>
                </v-row>
                <v-row>
                    <v-col>
                        <v-date-input label="Date and Time received by The Records Section"
                            prepend-inner-icon="$calendar" prepend-icon="" v-model="formData.dateReceivedRecordSection"
                    ...
                    <v-col>
                        <v-text-field v-model="formData.timeReceivedRecordSection" label="Pick Time" :active="timeMenu"
                            ...
                            <v-menu v-model="timeMenu" :close-on-content-click="false" activator="parent"
                                ...
                                <v-time-picker v-if="timeMenu" v-model="formData.timeReceivedRecordSection"
                                    format="24hr">
                                </v-time-picker>
                            </v-menu>
                        </v-text-field>
                ...
                <v-row>
                    <v-col>
                        <v-select :items="typesOfTransaction" item-title="transaction_type"
                            item-value="transaction_type" v-model="formData.typeOfTransaction" outlined
                ...
                <v-row v-if="prescribedPeriodValues.length !== 0">
                    <v-col v-for="(value, _) in prescribedPeriodValues">
                        <ScoreboardFormDialog @form-submitted="handleDialogFormSubmit"
                            :key="value.prescribed_periods_id" :report="value.report"
                            :prescribedPeriod="{ prescribed_period_value: value.prescribed_period_value, prescribed_period_id: value.prescribed_periods_id }" />
                ...
                <v-row>
                    ...
                    <v-col>
                        <v-select v-model="formData.status" :items="options.statuses" item-title="status_name"
                            item-value="id" label="Status" outlined></v-select>
                    </v-col>
                </v-row>
                <v-row>
                    <v-col>
