...
const displayValue = (val) => {
  // Checks if the value is NaN, zero, null, or undefined
  if (val === 0 || Number.isNaN(val) || val === null || val === undefined) {
    return '';
  }
  return val;
};
const handleRoleChange = (SelectRole) => {
  PMS_level.value = SelectRole.name;
};
const generateTable = async () => {
  ...
  if (!selectedQuarters.value || selectedQuarters.value.length === 0) {
   ...
   if (PMS_level.value === 'Senior BMS') {
    ...
    else if (PMS_level.value === 'Individual') {
      prescribedDay.value = 35;
    }
    else if (PMS_level.value === 'Supervising BMS') {
      prescribedDay.value = 40;
    }
  ...
  // Helper object to store clean, computed results per quarter
  const metrics = {};
  ...
  ['1', '2', '3', '4'].forEach(q => {
    ...
    if (!total || total === 0) {
      ...
      metrics[q] = {
        efficiency: eff,
        smp: eff === 100 ? 5 : 1 // Logic matching your rules
      };
    }
  });

  // 2. Calculate Averages safely (ignoring empty/blank strings)
  ...
  // Check if all 4 quarters are currently active
  ...
  // ⭐️ Only run calculation if allQuartersSelected is true, otherwise pass empty string
  const year_ave = allQuartersSelected 
    ? calculateAverage([metrics['1'].smp, metrics['2'].smp, metrics['3'].smp, metrics['4'].smp])
    : '';
//start-For period coverage
  if (selectedQuarters.value.includes('1')) { 
      from_date.value = 'January 1'
    } 
  else if (selectedQuarters.value.includes('2')) {
      from_date.value = 'April 1'
    }
  else if (selectedQuarters.value.includes('3')) {
     from_date.value = 'July 1'
    }
  else if (selectedQuarters.value.includes('4')) {
    from_date.value = 'October 1'
  }

   if (selectedQuarters.value.includes('4')) { 
          to_date.value = 'December 31'
    } 
  else if (selectedQuarters.value.includes('3')) {
      to_date.value = 'September 30'
    }
  else if (selectedQuarters.value.includes('2')) {
     to_date.value = 'June 30'
    }
  else if (selectedQuarters.value.includes('1')) {
    to_date.value = 'March 31'
  }
//end-For period coverage
console.log('Prescribed Day', prescribedDay.value);
  scoreboardData.value.push(
  {
    particular: 'Dimension: Efficiency',
    isBold: true 
    // 🔒 Locked: No allowEdit flag
  },
  {
    particular: 'Number of LGUs budgets reviewed within 35 calendar days (cd)',
  ...
  {
    particular: 'Number LGUs budgets reviewed within and beyond 35 cd',
  ...
  },
  {
    ...
    isBold: true
    // 🔒 Locked: Automated system rating row
  },
  {
    particular: '',
    customHeight: '20px' 
  ...
  },
  {
    ...
    isBold: true,
    isLarge: true,
    allowEdit: true // 🔓 Editable: Users can manually override final ratings
  }
);
};
...
<v-row no-gutters class="mt-2">
 ...
 <div id="printSection">
    <v-container>
        <v-row>
            <v-col>
                 <div><span class = "header-name">MONITORING TOOL 3: AGENCY PERFORMANCE REVIEW</span></div>
  ...
  <tbody>
    <tr v-for="(row, index) in filteredScoreboardData" 
        ...
        :class="{ 'text-bold': row.isBold, 'text-large': row.isLarge }"> 
